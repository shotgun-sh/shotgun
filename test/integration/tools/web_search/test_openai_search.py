...
def test_openai_web_search_smoke():
    ...
    try:
        result = openai_web_search_tool("What is Python programming language?")
...
@pytest.mark.skipif(not os.getenv("OPENAI_API_KEY"), reason="OpenAI API key not set")
def test_openai_web_search_with_specific_query():
...
def test_openai_web_search_without_api_key():
    ...
    try:
        ...
        # Tool should still be callable but return an error
        result = openai_web_search_tool("Test query")
        assert isinstance(result, str)
        # Check for the error message about missing API key
