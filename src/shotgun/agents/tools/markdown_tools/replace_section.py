...
async def replace_markdown_section(
    ...
    try:
        ...
        if confidence < 1.0:
            close = find_close_matches(headings, section_heading, threshold=confidence - 0.1)
        ...
        start_line, end_line = find_section_bounds(lines, matched.line_number, matched.level)
        ...
        logger.debug(
            "Successfully replaced section '%s' in %s", matched.text, filename
        )
