...
## TUI Testing Automation

The Shotgun TUI can be **fully automated for testing** using Textual's built-in testing framework. This allows testing without LLM API keys.

### Quick Start

**See full documentation:** `docs/TUI_TESTING.md`
...
@pytest.mark.asyncio
async def test_my_feature(screenshots_dir, mock_config):
    """Test TUI feature with screenshots."""
...
1. **Screenshot Capture**: Take SVG screenshots at any point (~16KB each)
2. **User Simulation**: Simulate key presses, clicks, navigation
3. **Widget Inspection**: Query and verify widget state
4. **Headless Mode**: Runs without terminal, perfect for CI/CD
5. **No API Keys Needed**: Tests work with mocked configuration
...
widgets = pilot.app.query("Button")
input_field = pilot.app.query_one("#input")

# Always pause after actions
await pilot.pause()  # Let app render
await pilot.pause(1.0)  # Longer pause (1 second)
```

### Available Fixtures

- **`screenshots_dir`**: Temporary directory for test screenshots
- **`mock_config`**: Mocked configuration (no API keys needed)

### When to Use TUI Tests

- Testing UI layouts and rendering
- Verifying keyboard navigation
- Testing screen transitions
- Visual regression testing
- Debugging UI behavior (via screenshots)
- Testing without LLM API keys

**IMPORTANT**: TUI tests don't make LLM API calls - they test the UI itself. For agent logic testing, use the appropriate integration tests with `@pytest.mark.llm`.
...
- **Run TUI tests**: `uv run pytest test/unit/tui/test_app_automation.py -v`
